...
4. **交付证据面核对**：`evidence.md` 存在且「入口与环境 / 过程证据 / 怎么验」三段非空（规范见 [../eo-shared/evidence.md](../eo-shared/evidence.md)）。缺失或必备段为空 = 校验失败 → 按封闭选择协议（[../eo-shared/questioning.md](../eo-shared/questioning.md) §4）三选一：**补齐**（回 implement 或 /eo-fix 循环内分支）/ **显式豁免**（§6 记录）/ **终止归档**——不静默放行
5. **归档前先发交付汇报，与确认提问同条消息发出**（封闭选择，跨会话最终复核）：
