...
- **判定完整性**：回收时要核的角色、工件、基线与判据归属；它不把总控变成 implement / test / review
...
- 一并行 worker 一**独立 worktree**（现场隔离，手段见基底文件）；同层全部收口后指派其一 worker 执行合并与合流 checkpoint，总控按下方四项检查校验其证据，冲突修复派回引入冲突批的原 worker
- 并行派发 >2 个 worker 前先报数量与预算，等用户点头（见事实说明）

校验按「回收」节读证据、不读 worker 的话，四项基本检查：
...
- **判定完整**：证据来自有权角色并锚定当前基线；implement 自检不得冒充节点契约要求的独立 test / review / manual 证据；无权改写 AC、测试锁或判据，以及把「未验证」包装成 PASS → 直接拒收。对「为换绿而弱化测试 / judge、针对样例硬编码、吞错」只有怀疑而无独立报告时，默认派独立 eo-review 审判据与 diff；仅缺“行为是否满足 AC”的执行证据时派 eo-test；两类同时存在可在同一基线上并行，仍保持角色隔离。总控不冒充代码审查下结论
...
拒收只触发上述打回 / 上交：总控不得为了补齐判定而亲自实施、改测试、执行 eo-test 或兼任 eo-review；需要实质判断时派给对应有权角色。
...
| 先校验再裁决 | worker 交付不直接采信：四项检查 → 通过 / 打回（附证据）/ 上交用户；拒收不授权总控兼任 implement / test / review，代用户做裁决 = 违规 |
