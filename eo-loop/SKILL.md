...
**② 选基底**：对线段上每个节点确定执行者与模型，三级优先：**本次用户显式指定 > 偏好文件 > 探测选定**。**基底无内置倾向**：运行时 `ls` 本 skill `references/substrates/*.md` 得到基底清单，读候选文件按其「探测」节确认可用；用户未指定且偏好未命中的节点，列出可用基底（附探测结果）供封闭选择。偏好层与探测选定均**不静默生效**：开跑前列出「节点 → 基底/模型」与依据问一次「按此跑？」；确认后收口按「经验沉淀」回写。
...
基底 = 「把一个节点交给谁执行」的载体。一基底一文件，放 `references/substrates/`，增减基底 = 加删文件；**基底之间无优先倾向**，可用性只由各文件「探测」节决定。枚举候选时 `ls` 得到 `references/substrates/*.md` 后**排除 `_template.md`**（它是模板不是基底）。每个基底文件必须含五节（照 `references/substrates/_template.md`）：**探测 / 派发 / 等待与观测 / 回收 / 已知陷阱**。「已知陷阱」节只放出厂陷阱——运行时新学到的坑记进基底陷阱库 `~/.eo-skills/loop/traps/<基底名>.md`（跨项目全局唯一，陷阱跟基底走、全项目自动继承），读基底文件时一并读。
...
根目录 `~/.eo-skills/loop/`，四个位置各司其主（文件格式与读写纪律见 [references/preferences-format.md](references/preferences-format.md)）：

| 位置 | 收什么 | 属性归属 |
|------|--------|----------|
| `preferences/<项目短名>.md` | 节点→基底/模型映射、习惯线段、项目纪律（用户明示的调度规则）、项目环境陷阱 | 项目 |
| `traps/<基底名>.md` | 该基底的操作陷阱（注入被吞、handle 漂移、弹窗挡提交等），全局唯一 | 基底 |
| `traps/_general.md` | 与基底无关的总控自身操作坑、模型/agent 行为坑 | 执行环境 |
| `history/<项目短名>.md` | 收口流水（「本次实际生效」记录），append-only，留最近 20 段 | 沉淀原料 |

- **读**：② 选基底第二优先级读 `preferences/_global.md` 与 `preferences/<项目短名>.md`（项目文件同名条目优先）——列出命中条目请用户确认后生效；① 圈线段判不出时查其「习惯线段」节；基底确定后读 `traps/<基底名>.md` 与 `traps/_general.md`，连同基底文件出厂陷阱节一并消费
- **写**：收口时把本次「节点 → 基底/模型」映射与关键事件 append 进 `history/<项目短名>.md`；同一映射在 history 连续 ≥2 次出现 → 聚合进 `preferences/<项目短名>.md`；用户明示（「以后 review 都用 X」）直接进 preferences。用户当场纠偏 → 立即写入。跨项目映射仅当用户明示或已在 ≥2 个项目文件出现才写 `preferences/_global.md`
- **陷阱跟基底走**：基底操作陷阱只写 `traps/<基底名>.md`（不进项目文件、不再需要 `[基底名]` 前缀）——A 项目踩的坑，所有用该基底的项目自动继承；总控自身操作坑（shell 引号吞字、工件修订纪律这类）与模型行为坑（某模型特有的出错模式）写 `traps/_general.md`；项目环境坑（端口占用、部署形态、构建工具）留在项目文件
- **性质**：提示而非保证——按偏好调度失败 → 回退探测，并把失败记进对应 traps 文件
- **短名一致性**：写入前先扫目标目录，存在与目标短名近似的名字（前缀包含或编辑距离 ≤2）时**先核项目身份**——读两个项目的 `.eo-project.json` 项目名与仓库根路径，一致才复用既有名；不一致（foo/food、api/apis 这类真实不同项目）→ 不得合并，列候选走封闭选择，禁止静默并入
- **体积纪律**：preferences 单文件 ≤4KB；超限先合并同根因条目，再按日期淘汰最旧
