...
| `changes.py`（384 行） | `parse_change_file` / `scan_all_changes` / `scan_all_changes_split` / `group_changes_by_divergence` / `parse_ac_section` / `parse_todo_section` / `parse_oq_section` / `count_ac` / `count_todo` | change.md 各节解析与多 worktree 扫描（同 id 取状态最高者、seq 撞号告警）；`scan_all_changes_split` 同 id 折叠单卡：一致副本合并、实质分叉按活动尺子（`_change_activity_epoch`：git 末次提交 + 目录树 mtime 取大，平手按状态→路径）取最新变体出卡、其余收 `forks`；以 main worktree 为基准过滤状态更低的过期候选（先于折叠） |
