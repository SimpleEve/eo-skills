...
| `changes.py`（301 行） | `parse_change_file` / `scan_all_changes` / `parse_ac_section` / `parse_todo_section` / `parse_oq_section` / `count_ac` / `count_todo` | change.md 各节解析与多 worktree 扫描（同 id 取状态最高者、seq 撞号告警） |
