...
- [x] AC-11 `eo-sync watch --project <路径>` 可在任意目录运行、只追平该项目；`eo-sync watch --all` 一轮内追平全部有效注册项目，且 watch 运行期间新注册的项目自下一轮起被纳入（验证：两个注册项目下跑 `--all` 观察各自 stub 追平；watch 挂起期间注册第三个项目，观察下一轮被纳入）
...
- **四态结果矩阵**（基线更新 / 重试 / 输出，逐格可测）：
  ...
  |----------|------|--------------------------|--------|
  | 0 全成 | run 后**重算**键，与 run 前键一致才记基线；不一致（第三方流转或自身回写，键粒度无法归因一律视为不能证明）→ 不记，下轮重 run 幂等追平——绝不吸收未投影的流转（test#1 FAIL-1 精化） | 键一致则短路静默；不一致复跑一轮后收敛 | 一行 run 诊断（项目 + 时刻） |
  | 1 部分失败 | 同上（适配器级失败自带告警；幂等重跑无害，不构成对持续性故障的忙循环） | 同上 | run 诊断 + run 自带的适配器告警 |
