...
- [ ] AC-1 同一 change 在两个 worktree 内容实质分叉（如各自勾选不同 TODO、或 status 不同）时，board 上该 change 显示为多张卡，各卡展示自己的状态与 AC/TODO 进度（验证：构造双 worktree fixture 分别改 change.md，跑 eo-board 看两张卡）（锁定：tests/test_eo_board_cache.py#BoardWorktreeSplitTests.test_ac1_diverged_change_shows_multiple_cards）
- [ ] AC-2 多个 worktree 中 change.md 内容完全一致的副本只出一张卡，不出现重复卡（验证：worktree 原样携带 change 目录副本时，看板卡数与拆分前一致）（锁定：tests/test_eo_board_cache.py#BoardWorktreeSplitTests.test_ac2_identical_copies_merge_to_one_card）
