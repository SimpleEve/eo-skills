...
- **实际做法**（两轮反馈后的终态）：`PROJECT_CSS`（serve 的 `/p/<key>` 与快照 `#/p/<key>` 泳道页）的 `.wrap` 去掉宽度上限走全宽；`ALL_HTML_TEMPLATE`（首页 change 流与概要卡两视图、快照 `#/` 与 `#/cards`）的 `.wrap` 为 `max-width: 1280px` 居中。两者均保留左右 24px 内边距与 479px 以下的既有窄屏档。全部 106 处 `font-size` 声明每档 +0.5px（含失效路由指引页；favicon 内嵌 SVG 的 `font-size='90'` 是图标字形尺寸，不在此列）。
- **原因**：AC-9 人工验收第 1 轮（基线 5c19342）用户要求「页面内容可以放到全宽，主要是下钻的视图，否则泳道展示不开」「整体的所有字号可以统一放大 0.5」——泳道页六列固定 268px + 14px 间距共约 1678px，在 1520px 上限下必然横滚，全宽是主诉求；当轮两页一并放开。第 2 轮（基线 ff0e3a9）用户按「列表 / 流式视图定宽、看板 / 泳道视图全宽」的惯例把首页收回 1280px 定宽，泳道页维持全宽。
