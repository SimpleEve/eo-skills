...
summary: >
  ...
  泳道页版面定格为列内滚动、列头吸顶，列可折叠成窄条并按项目记忆，Cmd/Ctrl+K 或 `/` 唤起定位搜索（`#编号` 直跳、全文命中片段，命中折叠列自动展开）；
...
conclusions:
  ...
  - 列折叠是布局偏好不是检索：列头眼睛图标收成只显列名+计数的窄条，按项目记忆（换项目互不干扰），刷新与 serve 热刷新后保持
  - 定位搜索只下钻页内：Cmd/Ctrl+K 或 `/` 唤起（输入框聚焦时 `/` 不误触），关键词搜标题/summary/全文与 backlog 卡正文，`#编号` 直达；选中后目标卡滚动高亮、他卡降透明，命中折叠列会自动展开；Esc 按搜索→详情→定位态逐层退出，热刷新后定位态清空
---
...
- 列头眼睛图标把该列收成 48px 窄条（竖排列名 + 计数），点窄条任意处展开还原；折叠状态按项目记忆（localStorage，键含项目路由键），刷新页面与 serve 3 秒热刷新后都保持；不同项目各记各的

### 定位搜索面板

- Cmd+K（mac）/ Ctrl+K 或 `/` 唤起（输入框聚焦时 `/` 不误触发），Esc 或点面板外关闭；Esc 按「搜索面板 → 详情抽屉 → 定位态」的已打开层级逐层消费
