...
- **维度 6 · 条件节合规**：命中风险信号却未在 §6 记录（含豁免）→ P1；defer 超 3 条 → P1。**信号复判**（jev 探测可用时）：按 [../eo-shared/jev-gate.md](../eo-shared/jev-gate.md) 挂钩点 1 以当前 §1/§2 独立判定三个语义类信号——jev 命中而 change.md 既未记录信号也未记录豁免 → P1（附 P(true)）；**起草时的 jev 结论仅在 §1/§2 未变且同 jev 版本时可直接引用**（有变即按当前 state 重跑，防止过期结论复用；闸门结论不跨 change 继承）；不可用或失败 → 跳过，不降级维度 6 其余检查
