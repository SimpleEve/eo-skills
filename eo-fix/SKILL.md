...
**jev 预判优先**（探测可用时）：按 [../eo-shared/jev-gate.md](../eo-shared/jev-gate.md) 挂钩点 2 以 manifest 摘录做五分类预判（jev 零共享上下文，结构性满足「新鲜眼睛」要求）——top-1 P ≥0.7 且与 top-2 差 ≥0.2 → 直接采纳，结论写 `卡点检查：<根因>（jev 预判，P=<top1>，<jev 模型版本>），<日期>`，按下表定出口，省掉 subagent；阈值不满足或 jev 不可用 → spawn **新鲜上下文 subagent**（执行者自述不作数——修了 3 轮的 agent 是判断自己为何修不好的最差人选），输入按 manifest 给，**禁止默认通读报告全史**：
...
结论一行写入 change.md 末尾：`卡点检查：<根因>，<日期>`（jev 预判采纳时带「jev 预判，P=<top1>」标注）。**失败关闭**：subagent 起不了（槽位/工具不可用）→ 不得用执行者自述替代，记「卡点检查未执行」，用户在「稍后重试 / 直接回炉 / 单轮豁免」中裁决。**jev 预判证伪**：按预判出口走一轮仍同位失败 → 视同一次打回，重进卡点检查（此时必走 subagent，不再 jev 预判）。
