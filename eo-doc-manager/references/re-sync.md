...
| 覆盖 design/ | 仅更新 impl_status | 仅更新 impl_status |
| 覆盖 research/ | 不涉及 | 不涉及 |
| 覆盖 knowledgebase/ | 不涉及 | 不涉及 |
| 适用场景 | 日常同步 | 文档与代码严重脱节、首次建立 |

## 完整流程

> **代码优先原则**适用于 re-sync。doc/ 和 agent-handbook/ 的内容从源码生成，不从已有文档迁移。已有文档仅作为"人工业务背景"的参考源。详见 SKILL.md「代码优先原则」。
...
### Step 4: 更新 design/ 实现状态

1. **不重建 design/ 文档内容**（设计是人写的）
2. 遍历所有 design 文档的实现状态表
3. 基于重建的 doc/agent-handbook 内容，全量校准状态
4. 更新所有 impl_coverage

### Step 5: 重建顶级 INDEX.md

汇总所有目录的文档信息。

### Step 6: 重置 sync-cursor

将 `.sync-cursor` 更新为当前 HEAD，为后续增量 sync 建立新基线：
...
doc/ (重建):
  📄 doc/task-engine.md
  📄 doc/content-monitoring.md
  共 N 篇

design/ (状态更新):
  📊 design/task-engine-spec.md — impl_coverage 3/5
  📊 design/monitoring-spec.md — impl_coverage 4/4
...
- re-sync 前向用户确认，因为会清空并重建 doc/ 和 agent-handbook/
- 重建时尽量保留已有文档中的人工补充内容
