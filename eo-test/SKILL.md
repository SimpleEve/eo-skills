...
2. **提取验证点**：读 change.md §2（AC 逐条）与 §3（TODO 完成判据），列出验证点清单；逐条标注 **light / heavy**（heavy = 起服务 / 多环境组合 / 点击流）与适合单测 / 需集成验证
...
对 **auto-heavy AC** 与其余不适合单测的 AC（用户视角操作链、端到端行为），按其「验证」栏描述的操作编写独立验证流程；捕获命令输出、退出码、运行结果作为执行证据。
