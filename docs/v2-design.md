...
7. **skill 是干件**。SKILL.md 及 references/ 正文只含操作指令与**运行时文件引用**（references/、eo-shared/ 等随包分发的文件，用可解析的相对路径）；禁止引用设计稿、决策编号、调研出处这类写作时的思路来源——设计思路的家在 docs/ 与 vault，不在交付物里。（临时例外：重构期的「v2 过渡状态」横幅，重写时随之删除。）
