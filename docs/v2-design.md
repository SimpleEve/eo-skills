...
- **跨模块判定移除**（决策 #3）：change 只挂项目。原「是否动别人的 spec」判界废弃；变更范围就是 TODO 行内文件（+ 条件节 §4 连带文件）+ AC 的辐射面。
...
- 每条 TODO 三要素：描述 / 涉及文件 / 对应 AC；完成判据是条件要素——仅多条 TODO 对同一 AC 时逐条写，一对一默认 = 批末对应 AC 验证通过（2026-07-18 定：竞品调研显示单条级判据是孤例，验证信息只在 AC 层写一遍）。**禁止占位符**（「add error handling」「后续完善」出现即拆解失败，借 superpowers）。
...
### Batch 2
- [ ] TODO-4 …（完成判据仅多条 TODO 对同一 AC 时写）

<!-- 以下为条件节，满足触发条件才写 -->

## 4. 涉及文件
触发：存在 TODO 未覆盖的连带影响文件（生成物、配置、迁移脚本）。
- path/to/file — <连带影响>
...
必填只有 §1-§3；模板全文（含条件节说明）控制在 ~80 行，对比 v1 的 217 行。
