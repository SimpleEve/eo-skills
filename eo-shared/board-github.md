...
> `eo-sync` 两个内置适配器（`eo-sync-obsidian` / `eo-sync-github`）投影 change frontmatter 的**内容契约**——本文件描述「投影成什么样」，不描述「何时触发」（触发已收敛为 archive 收口自动一次 + 手动 `eo-sync run`，逐流转投影已退役）。协议、发现与身份回写见 [../docs/sync-adapter-protocol.md](../docs/sync-adapter-protocol.md)。两适配器由 `.eo-project.json` 的 `sync` 段（或 `board` / `github` 段经兼容映射）**opt-in**；未启用即不投影、零成本。
...
## 一、Obsidian stub 适配器（`eo-sync-obsidian`；vault 模式，`board.enabled` 或 `sync.obsidian`）
...
## 二、GitHub issue（`eo-sync-github`；`github.issue: true` 或 `sync.github`）
...
## 三、GitHub PR（`eo-sync-github`，`github.pr` 策略；仅对 archived，archive 收口 run 生成）
...
`eo-project-init` 初始化（或后开时的更新分支）发现配置合并结果（`.eo-project.json` + 可选 `.eo-project.local.json` 覆盖）**缺失对应段**（区别于显式 `false`/`never`——那是用户已选择关闭）：
