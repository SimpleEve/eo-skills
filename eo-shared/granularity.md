...
**描述成本下界**（独立充分条件，不必四条全查）：说清怎么改所需的字数 ≈ 直接改掉的成本 → 直改。当描述一个改动比做它还贵时，任何工件都是净负收益。
